...
class GitHubAPIBackend(GitBackend):
    ...
    def __init__(
        self, timeout: int = 120, token: Optional[str] = None, verbose: bool = False
    ):
        ...
        try:
            ...
            if self._verbose and self._rate_limit_remaining is not None:
                ...
                if self._rate_limit_reset:
                    ...
                    reset_time_str = (
                        f" (resets {reset_dt.strftime('%Y-%m-%d %H:%M UTC')})"
                    )
            ...
            if resp.status_code == 403:
                ...
                if self._rate_limit_reset:
                    ...
                    reset_info = (
                        f" (resets at {reset_dt.strftime('%Y-%m-%d %H:%M UTC')})"
                    )
                auth_hint = (
                    " Set GITHUB_TOKEN or GH_TOKEN environment variable for 5000 req/hour."
    ...
    def find_oldest_commit_meeting_age(
        ...
        **kwargs,
        ...
        if verbose:
            print(
                f"[DEBUG] [github] owner={owner}, repo={repo}, ref={ref} -> resolved_ref={resolved_ref}",
                file=sys.stderr,
            )
            print(
                f"[DEBUG] [github] cutoff_ts={cutoff_ts} ({min_age_days}d ago)",
                file=sys.stderr,
            )
        ...
        if verbose:
            print(
                f"[DEBUG] [github] Querying commits until {cutoff_iso}...",
                file=sys.stderr,
            )
        ...
        if verbose:
            print(
                f"[DEBUG] [github] API response: status={status}, data_len={len(data) if isinstance(data, list) else 'N/A'}",
                file=sys.stderr,
            )
        ...
        if status == 200 and data:
            ...
            if ts is not None:
                ...
                if verbose:
                    print(
                        f"[DEBUG] [github] Found commit {commit.get('sha', '')[:8]} ts={ts}",
                        file=sys.stderr,
                    )
                return {
                    "ok": True,
        ...
        if status == 403:
            if verbose:
                print("[DEBUG] [github] Rate limited (403)", file=sys.stderr)
            return {
                "ok": False,
                "rev": None,
                "timestamp": None,
        ...
        if status == 404:
            if verbose:
                print("[DEBUG] [github] Ref not found (404)", file=sys.stderr)
            return {
                "ok": False,
                "rev": None,
                "timestamp": None,
        ...
        if verbose:
            print(
                "[DEBUG] [github] Fetching HEAD commit to report shortfall...",
                file=sys.stderr,
            )
        status, data = self._api_get(
            url,
        ...
        if status == 200 and data:
            ...
            if ts is not None:
                ...
                if verbose:
                    print(
                        f"[DEBUG] [github] HEAD is only {age_days:.1f}d old (needs {min_age_days}d)",
                        file=sys.stderr,
                    )
                return {
                    "ok": False,
